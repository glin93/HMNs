...
class Dataset(data.Dataset):
    ...
    def __init__(self, src_seq, trg_seq, index_seq, gate_seq,src_word2id, trg_word2id,max_len,ent,ID):
...
def collate_fn(data):
    ...
    src_seqs, trg_seqs, ind_seqs, gete_s, max_len, src_plain,trg_plain,ent,ID = zip(*data)
    ...
    if USE_CUDA:
        ...
        gete_s = gete_s.cuda()
    return src_seqs, src_lengths, trg_seqs, trg_lengths, ind_seqs, gete_s, src_plain, trg_plain,ent,ID
        ...
        for line in fin:
            ...
            if line:
                ...
                if '\t' in line:
                    ...
                    data.append([" ".join(contex_arr)+"$$$$",r,r_index,gate,ent,dialog_counter])
                    context+=str(r)+" " 
                else:
                    r=line
                    if USEKB:
...
def get_seq(pairs,lang,batch_size,type,max_len):   
    ...
    ID = []
    for pair in pairs:
        ### Filter words more than 660
        # if len(pair[0]) >600:
        #     warnings.warn('filter input sentence more than 660')
        #     continue
        x_seq.append(pair[0])
        y_seq.append(pair[1])
    ...
    dataset = Dataset(x_seq, y_seq,ptr_seq,gate_seq,lang.word2index, lang.word2index,max_len,ent,ID)

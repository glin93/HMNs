...
def read_langs(file_name, max_line = None,test=False):
    ...
    with open(file_name) as fin:
        ...
        for line in fin:
            ...
            if line:
                ...
                src_len.append(len(eng))
                tgt_len.append(len(fre))



    if test == True:
        max_r_len= max([len(u[1]) for u in data])
        max_len = max([len(u[0]) for u in data])
    else:

        src_dis_len = np.percentile(src_len, 80)
        tgt_dis_len = np.percentile(tgt_len, 80)

        print("top 95 % of src lens :", src_dis_len)
        print("top 95 % of tgt lens :", tgt_dis_len)


        print('Before filter traing len:',len(data))
        data  = list(filter(lambda line: len(line[0]) < src_dis_len and len(line[1])<tgt_dis_len ,data))
        print('After filter traing len:', len(data))
        max_r_len= max([len(u[1]) for u in data])
        max_len = max([len(u[0]) for u in data])
...
def prepare_data_seq(batch_size=100,shuffle=True,reload_vocab= False,path='data/mem2seq.txt' ):
    file_train = path

    pair_train,max_len, max_r = read_langs(file_train,max_line=None,test=reload_vocab)


    if reload_vocab == True:
        # read vocab from specific position
        print("Restoring lang vocab...")
        lang=  pickle.load(open('./save/lang.nmt.pkl','rb'))
    else:
        print("creating lang vocab...")
        lang = Lang()

    
    train = get_seq(pair_train,lang,batch_size,reload_vocab == False,max_len)

    if reload_vocab == False:
        print("Dumping lang vocab...")
        pickle.dump(lang,open('./save/lang.nmt.pkl','wb'))
